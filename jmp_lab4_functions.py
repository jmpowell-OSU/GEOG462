...
class SmartRaster(arcpy.Raster):
    ...
    def calculate_ndvi(self,  band4_index = 4, band3_index = 3):
        ...
        # Your code: NOTE I copied the code in lab3_functions_answers.py and tweaked it some
        
        try: 
            if arcpy.Exists(self):  # check to see if the image_name is there!
                
                # load just the NIR band into a raster object
                # Difference from lab 3: having to definine the full path of this raster
                # Other difference: calling bands. These are formatted as "Band_x" and are in thier own subdirectory of the raster
                nir = arcpy.Raster(arcpy.env.workspace + "\\" + self.raster_path+"\\Band_"+str(band4_index))

                # load just the RED band into a raster object
                red = arcpy.Raster(arcpy.env.workspace + "\\" + self.raster_path+"\\Band_"+str(band3_index))
        ...
        #your code:  LITERALLY copied/pasted from the lab 3 answer key, no changes needed (added some comments)
        try: 
            ndvi = (nir-red)/(nir+red)  # The math done
            return okay, ndvi   # Return takes the form of okay, ndvi just like the inputs from the _scripts file
...
class SmartVectorLayer:
    ...
    def zonal_stats_to_field(self, raster_path, statistic_type="MEAN", output_field="ZonalStat"):
        ...
        #Your code
        # Make a list of the fields in the feature class
        existing_fields = [f.name for f in arcpy.ListFields(self.feature_class)]

        #Check that list; does output_field "ZonalStat" exist in that list?
        if output_field in existing_fields:
            print(f"Field '{output_field}' already exists. Please choose a different name.")
            return False
        else:
            arcpy.management.AddField(self.feature_class, output_field, "DOUBLE") #Double is a float
        ...
        #  Your code
        try:
            arcpy.sa.ZonalStatisticsAsTable(self.feature_class, "OBJECTID", raster_path, temp_table, "DATA", statistic_type)
        except Exception as e:
        ...
        try:
            with arcpy.da.UpdateCursor(self.feature_class, ["OBJECTID", output_field]) as cursor:
                for row in cursor:
                    object_id = row[0]                      # take the object ID
                    if object_id in zonal_results:          # Does that object have a zonal statistic?
                        row[1] = zonal_results[object_id]   # Write zonal statistic into output_field
...
class smartPanda(pd.DataFrame):
    ...
    def scatterplot(self, x_field, y_field, title=None, 
                    ...
                    log_x=False, log_y=False):  # Added log x and y as options because of skewed parcel Acreage.
        ...
        if log_x:
            plt.xscale('log')  # Added becasue Acreage is skewed
        if log_y:
            plt.yscale('log')  # Added becasue Acreage is skewed
        plt.grid(True)
        plt.show()
    ...
    def save_scatterplot(self, x_field, y_field, outfile, title=None, 
                    x_min=None, x_max=None, 
                    y_min=None, y_max=None,
                    log_x=False, log_y=False):         # added because acreage is skewed
        """Make a scatterplot of two columns, with validation."""
        ...
        plt.grid(True)
        if log_x:
            plt.xscale('log')  # applies log scale to x axis
        if log_y:
    ...
    def plot_from_file(self, csv_control_file_path):
        ...
        try: 
            ...
            print(f"Problem reading the file: {e}")#{csv_control_file_path}")
            return False
        ...
        optional_params = ["x_min", "x_max", "y_min", "y_max", "log_x", "log_y"]
        ...
        for p in optional_params:
            if p not in param_dict.keys():
                param_dict[p] = None
            elif p in ["log_x", "log_y"]:                              # check for booleans first
                param_dict[p] = param_dict[p].strip().lower() == "true"  # convert before float check can grab it
            ## There was a problem with the provided code- it doesn't consider that numbers in the df may be stored as strings...
            elif param_dict[p] is not None:        # if the user did set it...
                param_dict[p] = float(param_dict[p])  # convert from string to number
        ...
        try:
            self.save_scatterplot(param_dict['x_field'], 
                               ...
                               log_x=param_dict['log_x'],                 # Added log x and y as options because of skewed parcel Acreage.
                               log_y=param_dict['log_y'])
