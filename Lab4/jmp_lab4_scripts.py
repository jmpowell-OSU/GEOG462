import os
os.chdir(r'R:\2026\Spring\GEOG562\Students\poweljer\Lab4_2025\Lab4\PythonCode\Lab4')
import arcpy
import pandas as pd
import matplotlib.pyplot as plt
import jmp_lab4_functions as l4
import importlib
...
arcpy.env.workspace = r"R:\2026\Spring\GEOG562\Students\poweljer\Lab4_2025\Lab4\lab4_arcproject_jmp\lab4_arcproject_jmp.gdb"
...
# The SmartRaster function is inheriting properties from arcpy.Raster, the parent function. 
# Specifically, we are inheriting  __init__ from arcpy.Raster, a function which loads in raster 
# properties including .height, .width, and .bandCount 
...
#  Your answer:
# The numbers 4 and 3 were hard-coded as inputs in the calculate_ndvi function. 
# When we called the variables in that function, we were literally calling "4" and "3"
...
# I found that there is no attribute for NDVI_mean in the new feature class, "Corvallis_parcels_plusNDVI", 
# however the NDVI_mean attribute DOES exist in the old layer "Corvallis_parcels"
# I like the way Claude explained the logic to me on this one:
"""Why Corvallis_parcels_plusNDVI Is Missing the Field
This comes down to operation order. Here's what happened:
save_as uses arcpy.management.CopyFeatures under the hood, which takes a snapshot of the feature class at the moment it's called. The sequence in your code was:

zonal_stats_to_field → writes NDVI_mean to Corvallis_parcels ✓
save_as → copies Corvallis_parcels to Corvallis_parcels_plusNDVI

So the copy should have included NDVI_mean. The likely culprit is that ArcPro was holding a lock on the feature class while you had it open in the map, preventing the cursor from fully committing all the updates before save_as ran.
This is a common ArcPy gotcha — if a layer is actively displayed in ArcPro while you're editing it via Python, Arc can interfere with writes."""

# I also found that the NDVI values in the raster DON'T DROP BELOW 0. 
# NDVI should range from -1 to 1, and though it's technically possible that no parcel 
# has a MEAN ndvi the negatives, I still suspect something is up.

# Furthermore, I'm finding lots of parcels that are just missing when symbolizing by NDVI.
# Calude tells me this is happening due to a limitation of using big Landsat pixels on small parcels. 
# That appears to be the case; missing parcels are small, and don't seem to encase any pixels entirely.
...
# The expectation is that the user is not passing a list of fields into the function. In that case, all fields in teh fc are
# being written into the output dataframe. If the user DID input a list of fields, that list is checked for accuracy and
# then the dataframe is built based only on the fields in that list. 
...
# Your answer:
# df_to_plot begins as the big dataframe "self". Then it gets filtered down one step at a time. 
# I like how claude explained it to me:
"""This is boolean filtering. Breaking it into parts:

*  df_to_plot[x_field] — grab the entire column specified by x_field
*  >= x_min — test every value in that column against the minimum, producing a list of True/False results for each row
*  df_to_plot[...] — use that True/False list to keep only the rows that returned True
*  The result is reassigned back to df_to_plot, replacing it with the filtered version

So if x_field is "NDVI_mean" and x_min is 0.1, any parcel row with an NDVI below 0.1 gets dropped from df_to_plot before plotting."""

# And doing the check for "x_min is not None" and for all the other varaibles ensures that the
# filter is only carried out if the user provided filter values
